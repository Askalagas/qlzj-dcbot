...
import { drawRunes } from '../engines/runeEngine.js'; 
import { narrateDivination } from '../divinationNarrator.js'; 
// NEW: Import getUserTitle
import { openai, trackUsage, canUse, PROMPTS, isChinese, getUserTitle, modifyMerit } from '../utils/shared.js';

export const data = new SlashCommandBuilder()
    .setName('rune')
    .setDescription('Cast a Rune reading')
    .addStringOption(option => 
        option.setName('question')
            .setDescription('Your question for the runes')
            .setRequired(false));

export const execute = async (interaction) => {
    if (!canUse(interaction.user.id, "rune")) {
        return interaction.reply({ content: "The runes have already been cast today.", ephemeral: true });
    }
    ...
    const runesDrawn = drawRunes(2);
    const narrative = narrateDivination("rune", runesDrawn);

    // 1. Detect Language
    const useChinese = isChinese(question);

    // 2. Determine Title (Pope vs Disciple)
    ...
    // 4. Select Instruction Template (With Title Injection)
    ...
    try {
        ...
        console.error(error);
        await interaction.editReply("The stones are silent. (API Error)");
    }
};